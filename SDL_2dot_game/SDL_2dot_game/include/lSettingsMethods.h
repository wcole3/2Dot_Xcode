...
void displaySettingsScreen(bool* globalQuit){
    ...
    while(!done){
        if(!*globalQuit){
            ...
            SDL_Rect textLine = {0,0, gWindow.getWidth() / 8, gWindow.getHeight() / 10};
...
void setHighlightBox(SDL_Rect* box, int x, int y){
    ...
    //the only special case is the back button which lives at x,y = 1,4
    if(x == 1 && y == 4){
...
void handleMouseEvent(SDL_Event e, int* xOffset, int *yOffset, bool* goBack, bool* globalQuit){
    if(e.type == SDL_MOUSEMOTION || e.type == SDL_MOUSEBUTTONDOWN){
        ...
        //need to test each possible textBox location excluding the back button
        for(int y = 0; y < 4; ++y){
            for(int x = 0; x < 2; ++x){
                ...
                if(checkMouseBoxCollision(xM, yM, textBox)){
                    ...
                    *yOffset = y;
                    ...
                    if(e.type == SDL_MOUSEBUTTONDOWN && *yOffset != 4){
                        changeSettingHotkey(e, *xOffset, *yOffset, globalQuit);
                    }
                }
            }
...
void changeSettingHotkey(SDL_Event e, int x, int y, bool* globalQuit){
    ...
    while(!keyEntered){
        ...
        gChangeSettingPrompt.render((gWindow.getWidth() - textLine.w)/2, (5 * gWindow.getHeight()) / 7, NULL, &textLine);
        SDL_RenderPresent(gWindow.getRenderer());
    }
    SDL_StopTextInput();
    if(keyChanged){
